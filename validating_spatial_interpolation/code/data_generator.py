...
def ensure_sensors_data(csv_path):
    """
    Ensures that the Geneva sensors dataset exists at csv_path.
    If not, automatically generates it deterministically.
    """
    if not os.path.exists(csv_path):
        os.makedirs(os.path.dirname(os.path.abspath(csv_path)), exist_ok=True)
        sensors = generate_geneva_dataset(n_sensors=100, seed=42)
